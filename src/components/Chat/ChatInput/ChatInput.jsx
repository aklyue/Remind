...
import { FaPaperclip } from "react-icons/fa"; // Импортируем иконку скрепки

export default function ChatInput({ sendMessage, setFile }) {
  const [value, setValue] = useState("");
  const [file, setLocalFile] = useState(null);
  const [loading, setLoading] = useState(false);
  const [progress, setProgress] = useState(0); // Для отслеживания прогресса загрузки
  const [uploadSuccess, setUploadSuccess] = useState(false); // Состояние для отображения уведомления
  ...
  const handleSend = () => {
    if (value.trim() || file) {
      ...
      setLoading(false);
      setProgress(0);
      setUploadSuccess(false); // Сброс состояния после отправки сообщения
    }
  };
